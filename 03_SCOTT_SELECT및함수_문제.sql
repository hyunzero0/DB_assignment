...
SELECT ENAME, EXTRACT(YEAR FROM HIREDATE) AS 년, EXTRACT(MONTH FROM HIREDATE) AS 월
FROM EMP;
...
SELECT EMPNO, ENAME, HIREDATE, HIREDATE + 365 * 40 -- 날짜로 40년 계산이라 날짜도 변경
FROM EMP;

SELECT EMPNO, ENAME, HIREDATE, HIREDATE + (INTERVAL '40' YEAR) -- 년도만 바뀜
FROM EMP;
