...
SELECT EMP_NAME, SALARY, ((SALARY + SALARY * NVL(BONUS, 0)) - SALARY * 0.03) * 12 AS 실수령액, HIRE_DATE
FROM EMPLOYEE
...
-- 19. EMPLOYEE테이블에서 근무 년수가 25년 이상인 직원 정보 조회
SELECT *
FROM EMPLOYEE
WHERE FLOOR(SYSDATE - HIRE_DATE) / 365 >= 25;
...
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 6),'RRMMDD')), 3, 2) || '년 ' || 
            EXTRACT(MONTH FROM TO_DATE(SUBSTR(EMP_NO, 1, 6),'RRMMDD')) || '월 '  ||
            EXTRACT(DAY FROM TO_DATE(SUBSTR(EMP_NO, 1, 6),'RRMMDD')) || '일' AS 생년월일,
            FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6),'RRMMDD')) / 365) || '살' AS 나이
            -- 생년월일 SUBSTR만 써도 가능
FROM EMPLOYEE
WHERE EMP_NAME NOT IN ('선동일', '송종기', '방명수', '옛사람');
-- 22. EMPLOYEE테이블에서 부서코드가 D5, D6, D9인 사원만 조회하되 D5면 총무부, D6면 기획부, D9면 영업부로 처리
--      (단, 부서코드 오름차순으로 정렬)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DECODE(DEPT_CODE, 'D5', '총무부', 'D6', '기획부', 'D9', '영업부') AS 부서명
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', 'D6', 'D9')
ORDER BY DEPT_CODE;
-- 23. EMPLOYEE테이블에서 사번이 201번인 사원명, 주민번호 앞자리, 주민번호 뒷자리, 
--      주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) AS 주민번호앞자리, SUBSTR(EMP_NO, 8,14) AS 주민번호뒷자리, SUBSTR(EMP_NO, 1, 6) + SUBSTR(EMP_NO, 8,14) AS 합
FROM EMPLOYEE
WHERE EMP_ID = 201;
...
COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2002, 1)) AS "2002년", COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2003, 1)) AS "2003년",
